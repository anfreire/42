 ...
 * @note restores the terminal to echo mode.
*/
Bot::~Bot()
{
 ...
 * 
 * @note Sets the terminal to non-echo mode.
 * @note Calls the init function.
 * @note Calls the connectToServer function.
 * @note Calls the checkActivity function.
 * @note Calls the recvMsg function.
*/
bool Bot::_run()
{
    struct termios term;
    ...
    while (confirm < 4)
    {
        ...
        if (msg.empty() || msg.find("Connection timed out") != std::string::npos)
            return false;
        if (msg.empty())
            continue;
